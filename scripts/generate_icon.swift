...
let outputDir = "LingXi/Assets.xcassets/AppIcon.appiconset"

// Base color: 10_blue (0.20, 0.48, 0.95)
// Subtle vertical gradient: slightly lighter top → base → slightly darker bottom
let baseColor: (r: CGFloat, g: CGFloat, b: CGFloat) = (0.20, 0.48, 0.95)
let lighten: CGFloat = 0.06
let darken: CGFloat = 0.08
...
func generateIcon() -> CGImage {
    ...
    // Very subtle top highlight (simulates light source from above)
    let glowColors = [
        CGColor(colorSpace: colorSpace, components: [1.0, 1.0, 1.0, 0.08])!,
        CGColor(colorSpace: colorSpace, components: [1.0, 1.0, 1.0, 0.0])!,
    ]
    if let glowGradient = CGGradient(colorsSpace: colorSpace, colors: glowColors as CFArray, locations: [0.0, 0.4]) {
        context.saveGState()
        context.addPath(squircle.cgPath)
        context.clip()
        context.drawLinearGradient(
            glowGradient,
            start: CGPoint(x: iconSize / 2, y: iconSize),
            end: CGPoint(x: iconSize / 2, y: iconSize * 0.4),
            options: []
        )
        context.restoreGState()
    }

    // Draw SF Symbol "atom" in white
    let nsContext = NSGraphicsContext(cgContext: context, flipped: false)
    NSGraphicsContext.saveGraphicsState()
    NSGraphicsContext.current = nsContext

    let symbolConfig = NSImage.SymbolConfiguration(pointSize: iconSize * 0.45, weight: .medium)
        .applying(.init(paletteColors: [.white]))
    if let symbolImage = NSImage(systemSymbolName: "atom", accessibilityDescription: nil)?
        ...
        shadow.shadowOffset = NSSize(width: 0, height: -iconSize * 0.01)
        shadow.shadowBlurRadius = iconSize * 0.025
        shadow.set()
